...
def get_mask_LDY(pre_mask, overlap):
    pre_mask = []
    pre_unique = np.unique(pre[..., 0])
    print(pre_unique)
    print(overlap)
    for i in pre_unique:
        print(i)
        mask = i == pre[..., 0]
        if (i==1 or i==2) or (i not in overlap): # i not in 중복
            pre_mask.append(mask.astype(bool))
            continue
        h, w = mask.shape
        max_area = (h * w) - mask.sum()
...
root= 'C:\\Users\\lab-com\\Desktop\\myspace\\BlenderProc_for_occlusion\\test230308\\'
...
for dir in file_list:
    ...
    '''
    class_inst_col_map = pd.read_csv(workspace + 'class_inst_col_map.csv')
    print(class_inst_col_map["category_id"])
    cat_id = class_inst_col_map["category_id"]
    print(cat_id)
    '''

    data = []
    f = open(workspace + 'class_inst_col_map.csv')
    class_inst_col_map = csv.reader(f)
    for row in class_inst_col_map:
        data.append((row[1]))
    f.close
    ...
    overlap_cat_id = []
    for c in data_counter:
        print(c)
        print(data_counter[c])
        if data_counter[c] > 1:
            overlap_cat_id.append(c)


    print(overlap_cat_id)
    overlap_cat_id = list(map(int, overlap_cat_id))
    print(overlap_cat_id)
    overlap_cat_id.sort()
    print(overlap_cat_id)
    '''
    ...
    for k in range(0, 20):
        ...
        m = 0
        n = 0
        for b in range(len(cat_id)):
            #print(cat_id[b])
            cat_unique = np.unique(cat_id[b])
            cat_unique = list(cat_unique)
            print(cat_unique)
            #print(segmap_next_0_unique == cat_unique)
            if(segmap_next_0_unique == cat_unique):
                m = m + 1
                n = b
