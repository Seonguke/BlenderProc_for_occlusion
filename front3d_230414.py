...
parser.add_argument("--front",default=None, help="Path to the 3D front file")
...
proximity_checks = {"min": 1.5, "avg": {"min": 2.5, "max": 7.0}, "no_background": True}
...
while tries < 50000 and poses < save_pictures:
    ...
    rotation = np.random.uniform([1.57079, 0, 0], [1.57079, 0, np.pi * 2])
    ...
    if bproc.camera.scene_coverage_score(cam2world_matrix, special_objects, special_objects_weight=10.0) > 0.8 \
        ...
        if len(arr) <= hide_obj_num or 'lighting' in arr[0].lower() or 'plants' in arr[0].lower() or 'plants' in arr[
            1].lower() or 'lighting' in arr[1].lower() or d[arr[0]] < 1:
            continue
        ...
        for i in range(hide_obj_num):
            # loc[arr[i]]=bpy.data.objects[arr[i]].location.z
            loc.append(bpy.data.objects[arr[i]].location.z)
            bpy.data.objects[arr[i]].location.z -= 100.0
        ...
        bpy.data.objects[arr[1]].location.z = loc[1]
        bpy.data.objects[arr[1]].keyframe_insert(data_path="location", frame=frame + 1)
        bproc.camera.add_camera_pose(room_id, cam2world_matrix)

        for i in reversed(range(hide_obj_num)):
            bpy.data.objects[arr[i]].location.z = loc[i]
            bpy.data.objects[arr[i]].keyframe_insert(data_path="location", frame=frame + 2)
        bproc.camera.add_camera_pose(room_id, cam2world_matrix)
...
'''
print(config)
print(config['modules'][0]['config']['output_dir'])
print(config['modules'][0]['config']['temp_dir'])
'''
